...
.PHONY: help setup-app setup-agents run-api run-worker run-feedback run-all stop-all logs-all ps restart-all
...
help:
	...
	@echo "  make run-all        # run all docker services (api/worker/feedback/redis/postgres)"
