...
from yolo_model.config.paths import (
    default_export_weights,
    default_highlight_weights,
    default_pose_output_dir,
)
...
def _app_dir() -> Path:
    # app/yolo_model/pipeline/runner.py → app/ bundle (agents, backendapi, yolo_model)
...
def resolve_highlight_weights() -> Path:
    ...
    candidates = [
        ...
        str(default_highlight_weights().relative_to(root)),
        str(default_export_weights().relative_to(root)),
    ]
...
def _output_root() -> Path:
    raw = (os.getenv("POSE_PIPELINE_OUTPUT_DIR") or str(default_pose_output_dir().relative_to(_app_dir()))).strip()
    root = Path(raw)
    if not root.is_absolute():
        if root.parts and root.parts[0] == "app":
