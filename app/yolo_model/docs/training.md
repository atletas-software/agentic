...
Single class: **`highlight_overlay`**. Labeling rules (from `docs/data_strategy.md`):
...
```bash
mkdir -p yolo_model/exports
cp yolo_model/artifacts/train/highlight_v1.1.0/weights/best.pt \
   yolo_model/exports/highlight_yolo_v1.1.0.pt
```
...
cp yolo_model/exports/highlight_yolo_v1.1.0.pt \
   agents/feedback/models/highlight_yolo_v1.pt
```

Then write a model card next to the weights — template lives in `docs/production.md` (section "Model card").
...
Run the new YOLO model alongside the legacy HSV detector for one week (see `docs/production.md` "Shadow deployment" for how). If diffs are within tolerance, flip `VIDEO_HIGHLIGHT_DETECTOR=yolo` to default in `app/backendapi/.env.example` and ship.

## 8. Save everything

Before walking away from a run, ensure these artifacts are committed or uploaded to your model registry:

- `yolo_model/exports/highlight_yolo_v<ver>.pt` (binary — git-ignored; upload to S3 / model registry)
- `yolo_model/exports/highlight_yolo_v<ver>.card.md` (model card — commit to git)
...
(This is what `VIDEO_HIGHLIGHT_YOLO_WEIGHTS` resolves to by default. Rolling back is a one-line symlink change — see `docs/production.md`.)
