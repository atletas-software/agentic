...
How to ship, validate, monitor, and improve the highlight-overlay detector in production. Read `docs/data_strategy.md` and `docs/training.md` first — this doc starts where they leave off.
...
Save as `yolo_model/exports/highlight_yolo_v1.1.0.card.md` (commit to git).
...
3. **Day 7**: 50%.
4. **Day 14**: 100%, flip default in `app/backendapi/.env.example`.
...
YOLO("yolo_model/exports/highlight_yolo_v1.1.0.pt").export(
...
- Never commit `.pt` files to git (`yolo_model/exports/.gitignore` enforces this).
- Always commit the model card (`.card.md`).
- Always commit the fixture manifest used for G2.
- Pin `ultralytics` and `torch` versions in `app/agents/requirements.txt`.
