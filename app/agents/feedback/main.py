...
# Repo-root .env when cwd is project root; then optional app/agents/.env overrides.
load_dotenv()
...
def _run_pose_review_job(review_id: str, payload: Dict[str, Any]) -> None:
    """YOLO pose JSON → one marker per red-circle highlight (vision + vector memory context)."""
    _save_job(
        review_id,
        {
            "id": review_id,
