...
from app.models.player_memory import SqlSyncCursor
from app.services.chunk_semantics import (
    CHUNK_TYPES,
    extract_tags,
    format_embedding_input,
    infer_chunk_type,
)
from app.services.evaluation_text_clean import format_sql_cell_for_embedding
from app.services.player_directory import upsert_player_directory_entry
from app.services.player_key import player_key_column_names, row_dict_from_sql
...
from app.services.player_summary import summarize_player_context
...
def _external_engine(database_url: str) -> Engine | None:
    ...
    if not raw:
        return None
    url = _normalize_external_db_url(raw)
...
def sync_sql_context_for_workspace(
    *,
    ...
    (not profile_id). Pinecone metadata follows player_memory_v2 (no raw HTML/JSON in text).
    """
    settings = get_player_memory_settings(db)
    eng = _external_engine(str(settings.get("sql_database_url") or ""))
    sql = str(settings.get("sql_query") or "").strip()
    ...
    if single_player is not None:
        ...
        if ":first_name" not in sql or ":last_name" not in sql:
            return {
                ...
                "detail": "Saved SQL must include :first_name and :last_name for per-player sync.",
            }
    with eng.connect() as conn:
        if sp:
            fn = str(sp.get("first_name") or "").strip()
            ln = str(sp.get("last_name") or "").strip()
            params: dict[str, Any] = {"first_name": fn, "last_name": ln}
    ...
    for row in rows:
        ...
        rid_int = _safe_int(rd.get("reviewee_id"))
        if rid_int <= 0:
            continue
    ...
    if sp:
        fn = str(sp.get("first_name") or "").strip()
        ln = str(sp.get("last_name") or "").strip()
        bind_used = {"first_name": fn, "last_name": ln}
        if not fn or not ln:
            return {
                "ok": False,
