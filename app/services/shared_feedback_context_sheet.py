...
def fetch_shared_feedback_context_text(*, max_chars: int = 14_000) -> tuple[str | None, dict[str, Any]]:
    ...
    Text is a compact tab-separated table string, truncated to max_chars.
    debug describes which spreadsheet / tab was used and the outcome (for transparency).
    """
    debug: dict[str, Any] = {
        ...
        "format_loaded": "Tab-separated: header row + data rows (newlines between rows). Injected as shared_context in the LLM user message.",
    }
    ...
    for raw_row in values[1:]:
        row = [str(raw_row[i]).strip().replace("\n", " ").replace("\t", " ") if i < len(raw_row) else "" for i in range(len(headers))]
