...
def ensure_env_loaded() -> None:
    ...
    primary = Path(configured) if configured else Path("app/backendapi/.env")
    root_env = Path(".env")
    ...
    if not loaded_primary and primary.resolve() != root_env.resolve() and root_env.exists():
        # Backward compatibility: only repo-root .env (no app/backendapi/.env)
        ...
        # Merge repo-root .env after app/backendapi/.env — fills POSTGRES_* etc. without overriding app keys.
        loaded_root = load_dotenv(dotenv_path=root_env, override=False)
    info(
        "env_loaded",
