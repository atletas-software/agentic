...
from backendapi.dependencies.player_memory_auth import get_player_memory_user_id
...
async def player_memory_status(
    ...
    backend = str(settings.get("vector_backend") or "firestore")
    stats = chunk_stats_by_scope(db=db, workspace_id=ws.id)
    return {
        "memory_enabled": ok,
        "backend": backend,
        "collections": stats.get("collections", {}),
        "total_chunks": stats.get("total_chunks", 0),
        "hint": None if ok else "Configure GCP Firestore vector memory (GCP_PROJECT_ID + ADC).",
    }
