...
# Firestore native vector search supports at most 2048 dimensions per vector.
FIRESTORE_MAX_EMBEDDING_DIM = 2048


def embedding_dimensions() -> int:
    raw = int(os.getenv("PLAYER_MEMORY_EMBEDDING_DIM", str(FIRESTORE_MAX_EMBEDDING_DIM)))
    if raw > FIRESTORE_MAX_EMBEDDING_DIM:
        raise RuntimeError(
            f"PLAYER_MEMORY_EMBEDDING_DIM={raw} exceeds Firestore limit ({FIRESTORE_MAX_EMBEDDING_DIM}). "
            f"Set PLAYER_MEMORY_EMBEDDING_DIM=2048 (text-embedding-3-large supports dimension reduction)."
        )
    return raw
