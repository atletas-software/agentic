"""Run feedback review jobs in-process (no separate feedback-agent HTTP service)."""
...
def run_feedback_review_inprocess(
    ...
    *,
    ...
    Writes job.json + review.json under agents.feedback DATA_DIR (same layout as the standalone agent).
    """
    ensure_directories()
    ...
    try:
        if text_only:
            review = _run_text_coaching(review_id, payload, on_progress=on_progress, cancel_check=cancel_check)
        elif use_pose:
            review = _run_pose_review(review_id, payload, on_progress=on_progress, cancel_check=cancel_check)
        else:
            review = _run_standard_review(review_id, payload, on_progress=on_progress, cancel_check=cancel_check)
