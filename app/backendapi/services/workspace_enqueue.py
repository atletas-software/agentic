...
from backendapi.services.sync_queue import get_workspace_queue
...
def enqueue_workspace_context_refresh(user_id: str) -> bool:
    ...
    try:
        ...
        q.enqueue(
            "backendapi.workers.workspace_worker.process_workspace_context_job",
            user_id,
            job_timeout=300,
...
def enqueue_feedback_delegate_job(agent_job_id: int) -> str | None:
    try:
        q = get_workspace_queue()
        rq_job = q.enqueue(
            "backendapi.workers.workspace_worker.process_feedback_delegate_job",
            agent_job_id,
...
def enqueue_video_processing_job(agent_job_id: int) -> bool:
    try:
        q = get_workspace_queue()
        q.enqueue(
            "backendapi.workers.workspace_worker.process_video_processing_job",
            agent_job_id,
...
def enqueue_destination_snapshot_embed_job(workspace_id: int) -> bool:
    try:
        q = get_workspace_queue()
        q.enqueue(
            "backendapi.workers.player_memory_worker.process_destination_snapshot_embed_job",
            workspace_id,
            job_timeout=600,
...
def enqueue_sql_player_sync_job(workspace_id: int) -> bool:
    try:
        q = get_workspace_queue()
        q.enqueue(
            "backendapi.workers.player_memory_worker.process_sql_player_sync_job",
            workspace_id,
            job_timeout=900,
            retry=Retry(max=1, interval=[30]),
        )
...
def enqueue_sql_player_sync_single_job(
    ...
    try:
        q = get_workspace_queue()
        q.enqueue(
            "backendapi.workers.player_memory_worker.process_sql_player_sync_single_job",
            workspace_id,
            player_id,
            first_name,
            last_name,
...
def enqueue_feedback_review_embed_job(agent_job_id: int) -> bool:
    try:
        q = get_workspace_queue()
        q.enqueue(
            "backendapi.workers.player_memory_worker.process_feedback_review_embed_job",
            agent_job_id,
            job_timeout=300,
