...
python -m yolo_model.scripts.pose_feedback \\
    ...
    --output runs/feedback/results.json

Full agent review (needs video + OPENAI_API_KEY, writes under agents/feedback/data):

PYTHONPATH=. python -m yolo_model.scripts.pose_feedback \\
    --pose-json results_yolov8.json \\
    --use-agent \\
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--kb", type=Path, default=None, help="Posture KB YAML (default: yolo_model/posture_guidelines.yaml)")
    ...
    kb = args.kb or (repo_root / "posture_guidelines.yaml")
