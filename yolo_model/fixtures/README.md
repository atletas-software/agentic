...
This folder holds **annotated test clips** that gate promoting the YOLO detector to the default (gate G2 in `PRODUCTION_PLAYBOOK.md`). The companion script `yolo_model/scripts/evaluate_pipeline.py` reads a JSON manifest from here and compares the pipeline's detected events to your hand-labeled ground truth.
...
Aggregate gate G2 (from `PRODUCTION_PLAYBOOK.md`): promote `VIDEO_HIGHLIGHT_DETECTOR=yolo` to default only once the fixture suite shows **recall ≥ 0.90** and **false-positive-event rate ≤ 0.05**.
